...
# t <- cont_east %>%
#   filter(Unit == "Antal" & Category %in% c("Fiske", "Plast") & name2 == "2013-2021 med interaktion")
# 
# ggplot(t, aes(factor(year), value, color = time_period)) +
#   geom_point() +
#   facet_wrap(~Category, ncol = 1)

# t %>% filter(year %in% c(2015, 2016))

# ggplot(t, aes(year, value, color = time_period)) +
#   geom_line() +
#   facet_wrap(~Category, ncol = 1)
...
sims_east %>% 
  ...
  geom_line(data = filter(cont_east, Unit == "Vikt"), aes(year, value, linetype = Signifikant, color = name2), size = 0.75) +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 5)) + 
  scale_color_manual(values = pal[c(2, 3)]) + 
  theme_plot() +
  theme(plot.margin = unit(c(0, 0.4, 0.4, 0.4), "cm"),
...
sims_west %>% 
  ...
  geom_line(data = filter(cont_west, Unit == "Vikt"), aes(year, value, linetype = Signifikant, color = name2), size = 0.75) +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 5)) + 
  scale_color_manual(values = pal[c(2, 3)]) + 
  theme_plot() +
  theme(plot.margin = unit(c(0, 0.4, 0.4, 0.4), "cm"),
